...
import { ContentResponse, postContent, updateContent } from '~/network/contents'
...
const CreateContent = ({ contentId, isUpdate, content }: CreateContentProps) => {
    const router = useRouter()
    const queryClient = useQueryClient()
    const formik = useFormik({
        initialValues: cleanDetails(content),
        validate,
    ...
    const container = useMutation((id: ObjectId) => getContainer(id), {
        onSuccess: (data) => {
            const fields = data.fields.map((field) => {
                const matching = content?.fields.find((f) => f.releatedFieldId === field.id)
