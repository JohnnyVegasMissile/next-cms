...
import { Content, ContentField, Metadata, Slug } from '@prisma/client'
...
const getContent = async (contentId: string) => {
    if (contentId === 'create') return { isUpdate: false }
    ...
    if (!content) return { isUpdate: true }
...
const CreateContent = async ({ params }: any) => {
    const { isUpdate, content } = await getContent(params.contentId)

    return <Form contentId={params.contentId} isUpdate={isUpdate} content={content || initialValues} />
}
