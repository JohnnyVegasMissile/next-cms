...
import { PlusOutlined, MinusOutlined, CaretUpOutlined, CaretDownOutlined, CloseOutlined } from '@ant-design/icons'
...
const Admin = () => {
  ...
  const addSection = () => {
    handleChange({
      target: {
        name: 'sections',
        value: [...get(values, 'sections', []), { type: undefined, content: '{}' }],
      },
    })
  }
      ...
      <Space direction="vertical" size="large" style={{ width: '100%', padding: 15 }}>
        <Space direction="vertical" size={0} style={{ width: '100%' }}>
          ...
          <Space direction="vertical">
            {get(values, 'metadatas', []).map((metadata: any, index: number) => (
              <Space key={index}>
                ...
                <Input
                  ...
                  onChange={(e) => onHandleChange(`metadatas.${index}.content`, e.target.value)}
                />
                <Button
            ...
            <Button onClick={addMetadata} type="primary" shape="circle" icon={<PlusOutlined />} />
          </Space>
                      ...
                      {idx === lastSlugIndex && (
                        <>
                          <Button
                            ...
                            type="primary"
                            shape="circle"
                            disabled={get(values, 'slug', '').split('/').length < 2}
                            icon={<MinusOutlined />}
                          />
                          <Button onClick={addSlug} type="primary" shape="circle" icon={<PlusOutlined />} />
                        </>
                      )}
                      <Input
                        value={slug}
              ...
              <div key={idx} style={{ display: 'flex', gap: 8 }}>
                ...
                <Card
                  title={
                    <Select
                      value={section.type}
                      onChange={(e) => onHandleChange(`sections.${idx}.type`, e)}
                      style={{ width: 250 }}
                    >
                  ...
                  style={{ flex: 1 }}
                >
                  <GetEditComponent type={section.type} defaultValues={section.content} onChange={() => {}} />
                </Card>
              </div>
            ))}
            <Button onClick={addSection} type="primary" shape="circle" icon={<PlusOutlined />} />
          </Space>
          <Divider />
